...
def get_csv():
    ...
    df.rename(columns={'greedy_output': 'MDG'}, inplace=True)
    df.rename(columns={'rpp_output': 'RPP'}, inplace=True)
    df.rename(columns={'luby_output': 'Luby'}, inplace=True)
    df.rename(columns={'blelloch_output': 'Blelloch'}, inplace=True)

    df.to_csv(os.path.join(sys.path[0], f"data_analyzed/" + "DIMAC" + ".csv"), index=False)
    df.to_pickle(os.path.join(sys.path[0], f"data_analyzed/" + "DIMAC" + ".pkl"))
