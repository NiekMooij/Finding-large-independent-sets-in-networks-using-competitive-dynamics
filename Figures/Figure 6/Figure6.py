...
plt.suptitle("Random geometric", fontsize=18, y=0.96)
...
# probability
df = pd.read_pickle(os.path.join(sys.path[0], f"data/random_geometric_probability.pkl"))

ax2.errorbar(df['r_radius'], df['LV_app'], yerr=(df['LV_app'] - df['LV_app_CI_lower'], df['LV_app_CI_upper'] - df['LV_app']), marker=markers[0], color=cud_palette[0], capsize=3)
ax2.scatter([], [], label='LV', marker=markers[0], color=cud_palette[0])

ax2.errorbar(df['r_radius'], df['greedy_app'], yerr=(df['greedy_app'] - df['greedy_app_CI_lower'], df['greedy_app_CI_upper'] - df['greedy_app']), marker=markers[2], color=cud_palette[2], capsize=3)
ax2.scatter([], [], label='MDG', marker=markers[2], color=cud_palette[2])

ax2.errorbar(df['r_radius'], df['rpp_app'], yerr=(df['rpp_app'] - df['rpp_app_CI_lower'], df['rpp_app_CI_upper'] - df['rpp_app']), marker=markers[3], color=cud_palette[3], capsize=3)
ax2.scatter([], [], label='RPP', marker=markers[3], color=cud_palette[3])

ax2.errorbar(df['r_radius'], df['luby_app'], yerr=(df['luby_app'] - df['luby_app_CI_lower'], df['luby_app_CI_upper'] - df['luby_app']), marker=markers[4], color=cud_palette[4], capsize=3)
ax2.scatter([], [], label='Luby', marker=markers[4], color=cud_palette[4])

ax2.errorbar(df['r_radius'], df['blelloch_app'], yerr=(df['blelloch_app'] - df['blelloch_app_CI_lower'], df['blelloch_app_CI_upper'] - df['blelloch_app']), marker=markers[5], color=cud_palette[5], capsize=3)
ax2.scatter([], [], label='Blelloch', marker=markers[5], color=cud_palette[5])

ax2.errorbar(df['r_radius'], df['continuation_app'], yerr=(df['continuation_app'] - df['continuation_app_CI_lower'], df['continuation_app_CI_upper'] - df['continuation_app']), marker=markers[1], color=cud_palette[1], capsize=3)
ax2.scatter([], [], label='CLV', marker=markers[1], color=cud_palette[1])

# ax2.legend()
ax2.set_xlabel('r')
ax2.set_ylabel('Approximation factor')
ax2.set_xticks([0, 0.3, 0.6, 0.9, 1.2, 1.5], ['0.0', '0.3', '0.6', '0.9', '1.2', '1.5'])
# ax2.set_yticks([0.60, 0.70, 0.80, 0.90, 1.0], ['0.60', '0.70', '0.80', '0.90', '1.00'])

# -------------------------------------------------------------------------------------------------------------------------------------------------

# size - sparse - performance
df = pd.read_pickle(os.path.join(sys.path[0], f"data/random_geometric_size_sparse_performance.pkl"))
...
df = pd.read_pickle(os.path.join(sys.path[0], f"data/random_geometric_size_dense_performance.pkl"))
...
# histogram
df = pd.read_pickle(os.path.join(sys.path[0], f"data/random_geometric_histogram.pkl"))
...
ax1.set_xticks([14, 16, 18, 20, 22, 24], ['14', '16', '18', '20', '22', '24'])
ax1.set_ylim(0, 0.7)
...
ax_inset.set_xticks([14, 18, 22], ['14', '18', '22'])
...
df = pd.read_pickle(os.path.join(sys.path[0], f"data/random_geometric_size_sparse_efficiency.pkl"))
...
df = pd.read_pickle(os.path.join(sys.path[0], f"data/random_geometric_size_dense_efficiency.pkl"))
...
df = pd.read_pickle(os.path.join(sys.path[0], f"data/random_geometric_size_sparse_worst_case.pkl"))
...
df = pd.read_pickle(os.path.join(sys.path[0], f"data/random_geometric_size_dense_worst_case.pkl"))
...
for index, ax in enumerate([ ax3, ax4, ax8 ]):
    ax.text(0.5, 1.05, r'$r = \sqrt{\frac{\log(n)}{\pi n}}, \langle k \rangle \approx \log(n)$', ha='center', transform=ax.transAxes, fontsize=12)    
    ax.tick_params(axis='x', labelsize=12)
    ax.tick_params(axis='y', labelsize=12)
    ax.set_xlabel( ax.get_xlabel(), fontsize=12)
    ax.set_ylabel( ax.get_ylabel(), fontsize=12)
...
for index, ax in enumerate([ ax5, ax6, ax9 ]):
    ax.text(0.5, 1.05, r'$r = 1/2, \langle k \rangle \approx \frac{n \pi}{4}$', ha='center', transform=ax.transAxes, fontsize=12)    
    ax.tick_params(axis='x', labelsize=12)
    ax.tick_params(axis='y', labelsize=12)
    ax.set_xlabel( ax.get_xlabel(), fontsize=12)
    ax.set_ylabel( ax.get_ylabel(), fontsize=12)

plt.savefig(os.path.join(sys.path[0], 'Figure6.pdf'), transparent=True, dpi=900, bbox_inches='tight')
plt.show()