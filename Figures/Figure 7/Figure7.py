...
plt.suptitle("Barabasi-Albert", fontsize=18, y=0.96)
...
ax2.errorbar(df['m']/60, df['LV_app'], yerr=(df['LV_app'] - df['LV_app_CI_lower'], df['LV_app_CI_upper'] - df['LV_app']), marker=markers[0], color=cud_palette[0], capsize=3)
ax2.scatter([], [], label='LV', marker=markers[0], color=cud_palette[0])

ax2.errorbar(df['m']/60, df['greedy_app'], yerr=(df['greedy_app'] - df['greedy_app_CI_lower'], df['greedy_app_CI_upper'] - df['greedy_app']), marker=markers[2], color=cud_palette[2], capsize=3)
ax2.scatter([], [], label='MDG', marker=markers[2], color=cud_palette[2])

ax2.errorbar(df['m']/60, df['rpp_app'], yerr=(df['rpp_app'] - df['rpp_app_CI_lower'], df['rpp_app_CI_upper'] - df['rpp_app']), marker=markers[3], color=cud_palette[3], capsize=3)
ax2.scatter([], [], label='RPP', marker=markers[3], color=cud_palette[3])

ax2.errorbar(df['m']/60, df['luby_app'], yerr=(df['luby_app'] - df['luby_app_CI_lower'], df['luby_app_CI_upper'] - df['luby_app']), marker=markers[4], color=cud_palette[4], capsize=3)
ax2.scatter([], [], label='Luby', marker=markers[4], color=cud_palette[4])

ax2.errorbar(df['m']/60, df['blelloch_app'], yerr=(df['blelloch_app'] - df['blelloch_app_CI_lower'], df['blelloch_app_CI_upper'] - df['blelloch_app']), marker=markers[5], color=cud_palette[5], capsize=3)
ax2.scatter([], [], label='Blelloch', marker=markers[5], color=cud_palette[5])

ax2.errorbar(df['m']/60, df['continuation_app'], yerr=(df['continuation_app'] - df['continuation_app_CI_lower'], df['continuation_app_CI_upper'] - df['continuation_app']), marker=markers[1], color=cud_palette[1], capsize=3)
...
df = pd.read_pickle(os.path.join(sys.path[0], f"data/barabasi_albert_size_dense_performance.pkl"))
...
ax5.set_xticks([0, 50, 100, 150], ['0', '50', '100', '150'])
...
ax1.set_xticks([12, 16, 20, 24], ['12', '16', '20', '24'])
ax1.set_ylim(0, 0.7)
...
ax_inset.set_xticks([12, 16, 20, 24], ['12', '16', '20', '24'])
...
df = pd.read_pickle(os.path.join(sys.path[0], f"data/barabasi_albert_size_dense_efficiency.pkl"))
...
ax6.set_xticks([0, 50, 100, 150], ['0', '50', '100', '150'])
# ax4.set_yticks([0.0, 0.25, 0.50, 0.75, 1.00], ['0.00', '0.25', '0.50', '0.75', '1.00'])
...
df = pd.read_pickle(os.path.join(sys.path[0], f"data/barabasi_albert_size_dense_worst_case.pkl"))
...
ax9.set_xticks([0, 50, 100, 150], ['0', '50', '100', '150'])
...
for index, ax in enumerate([ ax3, ax4, ax8 ]):
    ax.text(0.5, 1.05, f'({numbers[index]}) ' + r'$m = 5$', ha='center', transform=ax.transAxes, fontsize=12)    
    ax.tick_params(axis='x', labelsize=12)
    ax.tick_params(axis='y', labelsize=12)
    ax.set_xlabel( ax.get_xlabel(), fontsize=12)
    ax.set_ylabel( ax.get_ylabel(), fontsize=12)
...
for index, ax in enumerate([ ax5, ax6, ax9 ]):
    ax.text(0.5, 1.05, f'({numbers[index]}) ' + r'$m = n/2$', ha='center', transform=ax.transAxes, fontsize=12)    
    ax.tick_params(axis='x', labelsize=12)
    ax.tick_params(axis='y', labelsize=12)
    ax.set_xlabel( ax.get_xlabel(), fontsize=12)
    ax.set_ylabel( ax.get_ylabel(), fontsize=12)
