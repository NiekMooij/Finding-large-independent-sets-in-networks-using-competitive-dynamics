...
def analyse_graphs_type(type, save_after_n_runs=100):
    ...
    for index, row in df.iterrows():
        ...
        number += 1
        print(f'Network analysed - {type} - {number} - {datetime.datetime.now()}')

        if (count+1) % save_after_n_runs == 0 or index == len(df)-1:
            save_data(df, type)
            
def analyse_graphs(save_after_n_runs):
    types = [
    ...
    for t in types:
        analyse_graphs_type(type=t, save_after_n_runs=save_after_n_runs)